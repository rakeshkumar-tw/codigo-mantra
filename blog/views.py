...
def addComment(request, id):
  if request.user.is_authenticated:
    if request.method == "POST":
      ...
      newComment.save()
      newCommentObj = Comment.objects.get(id = newComment.id)
      isUserLike = False
      if newComment.likes.all():
        isUserLike = True
      # isUserLike = newCommentObj.likes.all()
      print("isUserLikeisUserLike",isUserLike)
      return JsonResponse({
...
def send_email(request, postId):
  if request.method == 'POST':
    ...
    recipient_email = response_json['email']
    html_template = get_template('send_email.html')
    blog = get_object_or_404(Article, pk=postId)
    context = {'blog_title':blog.title,'blog_content': blog.content}
    print("blog",blog)
    print("blogId ",postId)
    email_content = html_template.render(context)
    email = EmailMessage(
        f"Check out this blog: {blog.title}",  # Email subject
        email_content,         # Email content (HTML)
        "rakeshkumar.tw@gmail.com",  # Sender email address
